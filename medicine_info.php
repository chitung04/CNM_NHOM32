<?php
/**
 * Trang hiển thị thông tin thuốc từ QR code - KHÔNG CẦN ĐĂNG NHẬP
...
// Lấy QR code và medicine_id từ URL
$qrCode = $_GET['qr'] ?? '';
$medicineId = $_GET['medicine_id'] ?? '';

if (empty($qrCode) && empty($medicineId)) {
    http_response_code(404);
    die('❌ QR Code hoặc Medicine ID không hợp lệ');
}
...
try {
    ...
    // Tìm batch theo QR code hoặc medicine_id
    $sql = "SELECT b.*, m.medicine_name, m.price, m.description, c.category_name, u.unit_name, s.supplier_name
            ...
            WHERE b.qr_code = ? OR (m.medicine_id = ? AND ? != '')";
    
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$qrCode, $medicineId, $medicineId]);
    $batch = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$batch) {
        // Nếu không tìm thấy trong batches, thử tìm trong medicines
        $sql = "SELECT m.*, c.category_name, u.unit_name, 
                       0 as quantity, 0 as batch_id, m.qr_code as batch_number,
                ...
                LEFT JOIN categories c ON m.category_id = c.category_id
                LEFT JOIN units u ON m.unit_id = u.unit_id
                WHERE m.qr_code = ? OR (m.medicine_id = ? AND ? != '')";
        ...
        if (!$batch) {
            ...
            $batch = [
                'batch_id' => 0,
                'medicine_name' => 'Thông tin thuốc - ' . ($qrCode ?: 'ID: ' . $medicineId),
                'category_name' => 'Chưa phân loại',
                'unit_name' => 'Viên',
                'price' => 0,
                'quantity' => 0,
                'batch_number' => $qrCode ?: 'MED_' . $medicineId,
...
<head>
    ...
    <title><?php echo htmlspecialchars($batch['medicine_name']); ?> - Thông tin thuốc</title>
...
<body>
    ...
    <div class="modal fade show" id="medicineModal" tabindex="-1" style="display: block;" data-bs-backdrop="static" data-bs-keyboard="false">
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <div class="w-100">
                        ...
                        <h1 class="modal-title mb-2"><?php echo htmlspecialchars($batch['medicine_name']); ?></h1>
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-info mb-3">
                        ...
                        <br><small class="text-muted">
                            Quét QR code để xem thông tin chi tiết thuốc
                        </small>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('medicineModal').focus();
        });
